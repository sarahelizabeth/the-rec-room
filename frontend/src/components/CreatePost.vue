  ...
  <div class="card box">
    ...
    <form :class="showForm ? 'showing' : 'hidden'" method="POST" @submit.prevent="submitForm">
      <div class="card-content pt-2">
          ...
          <div class="control">
            <div class="select is-fullwidth">
              <select v-model="media_type" @change="getGenres($event)">
                ...
                <option v-for="option in mediaTypes" :key="option.id" :value="option.id" class="has-text-grey is-link">
                  {{ option.name }}
                </option>
              </select>
            </div>
        ...
        <div class="field">
          <div class="control">
            <input v-model="title" class="input is-link" type="text" placeholder="Recommendation title..." />
          </div>
        </div>
